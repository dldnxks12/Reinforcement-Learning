...
def main():
    ...
    # network type : simple | ensemble | noisy | jang
    net_name  = "ensemble"
    ...
    max_step     = 800000
    agent = td3.TD3(state_dim, action_dim, action_bound, device, d_sample, net_name, max_step, num_model)

    if net_name == 'ensemble':
        ensemble = True

    train = trainer.Trainer(env, env_eval, agent, env_name, device, d_sample, max_step, num_model, ensemble)
    train.run()
