...
# Agent
agent = SAC(env.observation_space.shape[0] + d_sample, env.action_space, delay_args, args)
...
for i_episode in itertools.count(1): # 1씩 증가시키는 무한 반복자
    ...
    while not done:
        ...
        if args.start_steps > total_numsteps:    # start_steps = default : 10000
            ...
            action = agent.select_action(I)[0]  # Sample action from policy
            next_state, reward, terminated, truncated, _ = env.step(act_buf[-d_sample].numpy())  # Step
            next_state = torch.FloatTensor(next_state)
    ...
    if i_episode % 10 == 0 and args.eval is True:
        ...
        for _  in range(episodes):
            ...
            while not done:
                ...
                action = agent.select_action(I, evaluate=True)[0]
